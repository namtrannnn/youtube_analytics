...
class KMeansTuyChinh:
    def __init__(self, so_cum=3, so_vong_lap_toi_da=100, dung_sai=1e-4):
        self.so_cum = so_cum
    ...
    def huan_luyen_va_du_doan(self, X):
        ...
        chi_so = np.random.choice(X.shape[0], self.so_cum, replace=False)
...
class HoiQuyLogisticTuyChinh:
    def __init__(self, toc_do_hoc=0.01, so_vong_lap=1000):
    ...
    def _ma_hoa_one_hot(self, y):
        so_mau = len(y)
        # Fix lỗi: Nếu chỉ có 1 class (vd toàn Positive), code cũ sẽ lỗi
    ...
    def huan_luyen(self, X, y):
        ...
        for _ in range(self.so_vong_lap):
            ...
            dao_ham_trong_so = (1 / so_mau) * np.dot(X.T, (y_du_doan - y_da_ma_hoa))
            dao_ham_do_lech = (1 / so_mau) * np.sum(y_du_doan - y_da_ma_hoa, axis=0)
            
            self.trong_so -= self.toc_do_hoc * dao_ham_trong_so
            self.do_lech -= self.toc_do_hoc * dao_ham_do_lech
